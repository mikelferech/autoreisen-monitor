...
export async function postResult(type,result){
  const url=process.env.MFE_WORKER_URL;const secret=process.env.MFE_MONITOR_SECRET;
  if(!url||!secret)throw new Error('Faltan MFE_WORKER_URL o MFE_MONITOR_SECRET en los secretos de GitHub.');
  const response=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${secret}`},body:JSON.stringify({action:'monitor-write',type,result})});
  const text=await response.text();if(!response.ok)throw new Error(`Worker ${response.status}: ${text}`);return text;
}
export function isoNow(){return new Date().toISOString();}
export async function runWithResult(type,fn){
  try{const result=await fn();await postResult(type,{...result,ok:true,status:'ok',checkedAt:result.checkedAt||isoNow()});console.log(`[${type}] OK`,result);return true;}
  catch(error){const result={ok:false,status:'error',error:error?.message||String(error),checkedAt:isoNow(),source:'GitHub Actions + Playwright'};console.error(`[${type}] ERROR`,error);try{await postResult(type,result);}catch(postError){console.error('No se pudo registrar el error en el Worker:',postError);}return false;}
}
